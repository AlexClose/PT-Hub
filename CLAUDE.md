...
**Current HEAD** `cf00676`: fixed the vanishing-supersets bug (see "Session log" below for full detail) — found and fixed the *actual* root cause (superset-unaware drag-reorder + `removeEx()`, not just Tim's stored data), not merely the workaround. Everything ships straight to `main` → Vercel. **Standing instruction from the user this session: push every fix/addition to `main` immediately, no holding changes on a review branch — the user checks on their phone as we go and will say if something needs reverting.**
...
**Not yet independently confirmed:** the user has not yet re-tested Tim's actual program on their phone since this shipped. If Tim's *specific* stored data turns out to have some other corruption shape not covered by the three tested scenarios, report back with what's seen (screenshot of the day in both the normal editor and a fresh Generate) and pick this thread back up — the diagnostic method (offline Node harness copy-pasting the pure functions, no dataset/DOM/Supabase needed) worked well and is reusable.
...
- ✅ **Generate Next Program — vanishing supersets, RESOLVED.** See the "Generate Next Program: supersets not surviving generation — RESOLVED" section near the top of this doc for full root-cause detail. Short version: the real bug was the program editor's drag-reorder (`setupExDrag`) and `removeEx()` being unaware of superset grouping — either could separate a paired exercise from its partner while both kept the same group tag, corrupting `structured_program`. The generator's adjacency-based grouping then (correctly, per an earlier fix) demoted these corrupted pairs to plain exercises, which is what looked like "zero supersets." Fixed with a new `_coalesceSsGroups()` helper (rescues scattered-but-real pairs, used by both the generator and the normal editor so they can't disagree again), a superset-aware rewrite of the drag-reorder (group handle moves the whole pair; a row's own handle only reorders within its group), and a self-healing `removeEx()`. Verified offline against three scenarios (scattered-real-pair, genuinely-orphaned-solo-tag, normal-pair) — commit `cf00676`. **Not yet confirmed against Tim's actual live data on the user's phone** — flag it back here if it's not fully resolved on re-test.
...
**Last Updated:** HEAD `cf00676` — new session that picked up the vanishing-supersets bug left open at the end of the prior session. Found the actual root cause (superset-unaware drag-reorder + `removeEx()` in the program editor, not the "Tim's data is pre-corrupted" hypothesis the prior session left as its next diagnostic step) and shipped a fix, verified offline against three scenarios — see "Generate Next Program: supersets not surviving generation — RESOLVED" near the top of this doc. **User has not yet re-tested Tim's actual program on their phone** — if it's not fully resolved, that's the thing to pick up next. **Standing instruction from the user this session:** push every fix/addition straight to `main` as it's shipped (no holding on a review branch) — they're checking along the way on their phone and will flag anything that needs reverting. **User's testing loop:** iPhone PWA, tests real client data (a client named "Tim" is the current test case for the generator), reports back with screenshots — implement, ship to main, they re-check on the phone.
