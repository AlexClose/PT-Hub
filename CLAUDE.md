...
**Current HEAD**: custom-workout `⠿` handle layout done; **drag-to-reorder still NOT working** on the user's iPhone (see 🚧 entry in Session log — recommended next action is ▲/▼ buttons).
**File**: `index.html`, ~7790 lines (line numbers shift constantly — ALWAYS grep, never trust the numbers in this doc)
...
- 🚧 **Custom-workout drag-reorder — STILL BROKEN, NEEDS A NEW APPROACH.** Goal: drag `⠿` to reorder exercises in the custom-workout builder, like the program editor already does.
  - **What works:** the full-height `⠿` handle **layout** is correct (rows are `display:flex` with a left handle strip in both `_addCustomExRow` client ~line 5155 and `addAdHocExercise` coach ~line 3160). The **program editor's `setupExDrag`** (~line 2826, on `#editExerciseList`, pointer events + reparent to `<body>` + array re-render) **WORKS on the user's iPhone.**
  - **What's broken:** dragging in the custom-workout sheet does NOT move rows on the user's iPhone — through EVERY approach tried this session (do not just retry these):
    1. Pointer events, capture on handle + `pointer-events:none` (bug).
    2. Pointer events, capture on the moved block, reparent to `<body>` (mirrors `setupExDrag`).
    3. Pointer events, **no reparent**, finger-follow transform + live reshuffle.
    4. **Touch events** (`touchstart`/`touchmove` preventDefault) + mouse, `-webkit-user-select/-touch-callout:none` on handle (symptom there: handle just text-highlighted blue).
    5. Guarded the `#workout-sheet` **swipe-to-dismiss** handlers (~lines 5129 & 5886) with `if(_blockDrag)return` so they don't steal the touch.
    Current code (`_setupBlockDrag` ~line 2882) = approach #2 + the swipe guard + `var _blockDrag` flag. User confirms layout updated but **drag still does nothing**.
  - **Why the program editor works but this doesn't is still unexplained.** Prime suspects for the next chat to actually DIAGNOSE (don't blind-fix): (a) is `pointerdown` even firing on the handle inside `#workout-sheet`? add a temporary `console.log`/on-screen debug and have the user report; (b) does `_setupCustomDrag()` run and find `#custom-ex-list` (it's called at ~line 5172)? (c) something in the sheet's scroll/`-webkit-overflow-scrolling:touch` container (CSS ~line 262) or another ancestor handler still eating pointer/touch events.
  - **✅ RECOMMENDED NEXT ACTION (do this first):** stop fighting the gesture — add small **▲/▼ tap buttons** to each row (in `_addCustomExRow` client + `addAdHocExercise` coach). On tap, swap the row with its previous/next `[data-dragitem]` sibling in the same container via `insertBefore`. Both savers read rows in **DOM document order**, so reordering the DOM is all that's needed — guaranteed to work on every device. Keep the `⠿` handle too if drag ever gets fixed, or replace it.
