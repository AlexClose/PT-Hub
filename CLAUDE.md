...
**Current HEAD** `723de4b` (index.html is at the `5384720` content — see the revert note). This latest session shipped several small **functional** fixes to `main`, then attempted a big **visual** dashboard redesign that the user rejected and I fully reverted. Where things stand:
- **Functional work this session, all live on `main` and kept:** dashboard session alerts are tappable → open the client's profile (`cf82689`); shared-package clients no longer trigger a false "low sessions" dashboard alert (`2b39f29`); coach log screen auto-fits long comma-separated values + deleting a session now restores one to the client's remaining count (`00fab84`); "Seated Hamstring Curl" added to the exercise catalog (`2fde8d4`); client **Kathryn**'s program replaced via a Supabase SQL UPDATE (no code change — see session log).
- **Visual work this session, REVERTED:** a full "data-lux" (Whoop/Oura) dashboard overhaul (`2fe96e7`) was built and shipped, but on the user's phone with real data it "looked worse — a lower-end, very basic app." Fully reverted (`425a945`); `index.html` is byte-for-byte back to `5384720`. **The user is pursuing the redesign a different way and does NOT want it re-attempted unless they explicitly ask.** Key lesson recorded in the session log: the headless-render previews looked good in isolation but did not predict the on-device feel — the user's phone is the only real signal for visual work here.
- **Everything ships straight to `main` → Vercel.** Standing instruction: push every fix/addition to `main` immediately; the user checks on their phone and says if something needs reverting. **Always confirm `git rev-parse HEAD` == `origin/main` before editing** — this remote web workspace has opened on a stale dev branch before (see the tappable-alerts session-log entry).
...
- ↩️ **REVERTED the data-lux dashboard overhaul** (`425a945`). Seen live on the user's phone with real data (30 clients), the redesign "just looks worse — a lower-end, very basic app." User asked to reset to before the change and said they'll pursue the redesign a different way. `index.html` restored byte-for-byte to `5384720` (pre-`2fe96e7`); dashboard is back to its prior look. CLAUDE.md history kept. **Lesson for any future UI attempt here: the headless-render harness previews read well in isolation but did NOT predict how the result felt on-device against the real roster + moving background — the user's on-phone reaction is the only real signal. The current/prior dashboard look is what they want; do not re-attempt this redesign unless the user explicitly asks.** The entry below documents what the (now-reverted) overhaul contained, for reference only.
- 🎨 **[REVERTED — see above]** UI overhaul "data-lux" (Whoop/Oura) dashboard redesign (`2fe96e7`, reverted in `425a945`). User wants the whole app to look deliberately designed / premium, not "built by AI / thrown together." Approach agreed: overhaul ONE page first (coach Home/dashboard), then propagate the system app-wide. Direction chosen by user: **Whoop/Oura data-lux** (dark, data-forward, big rings/numbers, soft gradients). **Method (IMPORTANT — do NOT ship visual work blind):** iterated in a headless-Chromium harness through v1→v4 against the app's REAL extracted CSS before touching index.html; user reacted at each step (v1/v2 "not different enough" → added structure; v3 added the signature weekly bar chart → "pretty clean"; v4 stronger section headers). Harness pattern in scratchpad: `app2.css` (extract the first `<style>` block from index.html) + a mock-markup html mirroring render output + `shot.js` (chromium at `/opt/pw-browsers/chromium-1194/chrome-linux/chrome`, `NODE_PATH=/opt/node22/lib/node_modules`). **What shipped (dashboard only, functionality unchanged, ambient particle bg preserved):** namespaced `dh-*` CSS classes + a subtle global film-grain (`body::after`, z:3); restructured `#view-dashboard` into top-bar → hero → week-chart → KPI-rail → sectioned (Needs Attention / For You / Clients) with color-coded `.dh-sec` accent bars. `renderTodaySessions` → Oura-style % ring hero + quick-log avatar chips (existing `quickLog`) + preserved expandable detail. New `renderDashWeek()` = weekly sessions bar chart (real data, Mon-start). `renderDashboard` mkStat→`kpi()` rail, alerts→`_attRow` action rows (still `openClientDetail`), insights→`.dh-mod`, section show/hide + header meta (date kicker, client pill/count). `clientRowHtml` → `.dh-crow` (avatar + status pill + color count + progress meter). **NEXT: user wants this same system rolled out to the rest of the app** (client modal, client portal, Library/Programs/Business, log screen already premium). Learn-Today card still uses its older green-editorial styling inside the new "For You" section — fine for now, refine when propagating. Not yet confirmed on user's phone as of shipping.
- ✅ **Dashboard session alerts are now tappable → open the client's profile** (`cf82689`). User wanted to tap a name in a low/out-session alert to jump straight to that client to edit (renew/adjust remaining). All four alert templates in `renderDashboard` (individual out, individual low, shared-package out, shared-package low) got `onclick="openClientDetail('<id>')"` + `cursor:pointer` + a right-chevron affordance (`_alertChev`); the global `div[onclick]:active` dim gives native press feedback. Shared-package alerts open the first linked client (guarded when none). **Process note:** the web session's working copy was checked out on the long-stale dev branch `claude/training-app-review-ghcqdl` (36 commits behind `main`, missing this session's entire log-screen redesign etc.); a first commit accidentally landed there — caught it via `grep` for recent features + `git merge-base`, reset to `origin/main`, re-applied on `main`, shipped. **Always confirm HEAD is at `origin/main` before editing/committing in this repo.**
- ✅ **Added "Seated Hamstring Curl" to the exercise catalog + machine-name aliases** (`2fde8d4`). User: seated hamstring curl wasn't surfacing when typed. The `Seated_Leg_Curl` demo alias and the hamstrings muscle-mapping already existed — the only gap was the autocomplete catalog (`_EX_SUGGEST` had "Seated Leg Curl" but not the "hamstring curl" phrasing). Added "Seated Hamstring Curl" + "Lying Hamstring Curl" to `_EX_SUGGEST`, plus explicit `…machine`-suffix aliases (seated variants → `Seated_Leg_Curl`, confirmed a real demo id in free-exercise-db so it animates and tracks hamstrings).
- ✅ **Replaced client "Kathryn"'s program via Supabase SQL (no code change).** User wrote a new 4-day program (Push / Pull / Legs-Quad / Glutes) and didn't want to hand-enter it in the app. I formatted it into the `structured_program` JSONB shape (`{started_at, days:[{name,color,exercises:[{name,prescription,load}]}]}`, supersets encoded as `"SS1: Name"` with per-day SS numbering; day colors from the `DAY_COLORS` cycle), verified the superset grouping offline, and handed the user a Supabase SQL `UPDATE clients SET structured_program = jsonb_build_object(...)::jsonb WHERE id='c6a1809f-1b4c-4f5c-8ec5-b6e2ae8197b7'` — keyed on Kathryn's **id** (a SELECT confirmed the exact stored spelling "Kathryn"; the name had bounced between Kathryn/Katherine/Catherine in chat). User ran it → "Success. 1 row affected." Loads left blank for her to fill in as she trains. **Reusable pattern for any "format my program for me" request: build the JSON, hand the user a SQL UPDATE keyed on the client id — Supabase is firewalled from this env, so I can't write it directly; the app itself has no bulk-program-import UI.**
- ✅ **Dashboard false "low sessions" alert for shared-package clients — FIXED** (`2b39f29`). User: home screen said "Chase has 2 sessions left — time to renew" but his profile (and his dad's — they share a combined package) showed 8. Root cause: `renderDashboard`'s `withPkg`/`low`/`out` filters (which drive both the alert list AND the Low-Sessions/Invoice-Needed stat tiles) used each client's individual `c.remaining`, but shared-package members' real count is `sharedPkg.remaining_sessions` — their individual `remaining` is stale. Fix: excluded `c.shared_package_id` clients from those three individual filters (they're already covered by the dedicated "Shared package alerts" loop and their profile/row badges use the shared-aware `displayRemaining`); also added `sharedLow`/`sharedOut` so genuinely-low/out shared packages count toward the stat tiles once per package. Individual-package clients unaffected.
- ✅ **Log screen: auto-fit long values + delete-session restores the count** (`00fab84`). Two user-reported fixes. (1) **Overflow**: long comma-separated values (`135,175,205`) overflowed the 22px `.log-input` and clipped with no scroll. New `_fitLogInput(el)` steps font down by content length (22→19→16.5→14→12px); wired into `checkExRowCoach` (on type), a post-render `#logExercises .log-input` pass (covers ghost placeholders + prefilled), and ad-hoc rows. Verified in harness that `135,175,205` and even `135,175,205,225,245` fit. (2) **Delete restores remaining**: `deleteEditSession` now mirrors the log-time decrement — individual `package_size>0||remaining!==0` → `remaining+1`; shared → `remaining_sessions+1` capped at `total_sessions`; pure pay-per-session (never charged) left alone. Confirm dialog states whether a restore will happen; toast confirms "1 added back to remaining". Matches the user's scenario (4 remaining → double-log → 2 → delete one → 3).
- ✅ **Log session screen — progress-bar integration + header anchoring** (`f497ffe`), two tweaks on the chrome pass below. (1) The progress bar sat in a sticky near-opaque dark band (`rgba(10,13,18,0.97)`+blur+border) that covered the modal ambient and looked like a disconnected block — dropped the sticky band so the label + thin 7px bar sit transparently on the page (integrated). (2) The client name floated as loose text — wrapped the header (avatar + LOG SESSION/name + lightning) in a subtle glass panel (gradient + hairline border + soft shadow) so it reads as a proper header. Verified in the headless harness (`preview5.html`).
- ✅ **Log session screen — full premium chrome pass** (`b7bab6c`), extending the card redesign to the whole screen (user: "the rest of it needs that same kind of look… the whole page needs that good glossed-over look… I want people to say wow"). Functionality untouched. **Header**: client avatar (`_avatarStyle(c,48)` injected into `#logModalAvatar` in `openLogSession`) beside the LOG SESSION/name block + the existing lightning tile. **Day selector**: tinted gradient card (blue left accent + dumbbell-icon tile + 16px Syne day name) wrapping the SAME `#logDaySelect` `<select>`/onchange — options given explicit dark bg for the native iOS dropdown. **Progress**: 7px rounded bar with green glow + Syne bright-number/muted-total count (`checkExRowCoach` now writes the two-tone via `innerHTML`). **Fill Last Session**: gradient pill + lightning glyph. **Mark Complete**: hero CTA via new `.btn-log-complete` class (gradient+glow+inset highlight; base look in CSS so `openLogSession`'s `markBtn.style.background=''` reset restores it, and the error-path red inline bg still overrides), Cancel narrowed to 40%. Verified the full assembled screen in the headless harness (`preview4.html`/`css_full.css`) before shipping. **Client-portal logging still on the old look — mirror both the cards AND this chrome there once the user is happy.**
- ✅ **Session-logging screen — BOLDER full redesign (numbered two-zone cards)** (`a628ea2`), superseding the too-subtle first pass. User rejected the first attempt ("I don't even really see what changed… still pretty plain… needs a full redesign, not just changing a few words/logos") and called the crosshair "target" chip confusing. Rebuilt the coach log exercise cards structurally (functionality untouched — all ids/handlers/placeholders/badges/guide preserved): **numbered `.log-num` chip** per exercise; **two-zone card** — info zone (number + 46px thumb + 18px name, then a spelled-out **"TARGET"** labeled reference replacing the confusing icon chip) on the card surface, then the inputs drop into a **distinct darker `.log-body` zone** with a top divider; bigger labeled stat-fields (`.log-field`/`.log-input`, 22px Syne values, focus glow, ghost last-session placeholder, green-when-filled via `:not(:placeholder-shown)`); **`.done` class greens the number chip + values + body zone**; ✓/PB badges sit in a reserved top-right corner (`.log-ex-head{padding-right:58px}`) clear of the name; the progression guide is preserved in `.log-guide-wrap` between target and inputs; superset members reuse the same structure inside the orange wrapper. **KEY PROCESS LESSON: this is a visual task — do NOT ship blind. Iterated in a headless-Chromium harness (scratchpad `preview3.html` + `css_real.css` extracted from index.html + `shot3.js`; `/opt/pw-browsers/chromium-1194/chrome-linux/chrome`, `NODE_PATH=/opt/node22/lib/node_modules`) and eyeballed the actual render (idle/done/PB/superset) BEFORE committing.** The first (rejected) pass `91be19c` was shipped without rendering it — that's why it underwhelmed. **Client-portal logging (`renderWorkoutCard` ~7920) still on the old look — mirror this once the user approves the direction.**
- ✅ **Session-logging screen redesign — premium cards + real input fields** (`91be19c`), coach log modal. [SUPERSEDED by `a628ea2` above — too subtle, user didn't notice the change.] User: the log screen "looks like a colored text document you could've built easily," wanted it to feel like a professional app; functionality is fine, **aesthetics only**. Preserved every id/handler/placeholder/thumbnail + the progression-guide card. New shared CSS classes: `.log-ex-card` (gradient surface + hairline top-highlight + depth shadow, `.done` variant = green tint/border/glow), `.log-ex-head`, `.log-target` (crosshair-SVG + pill replacing the loose blue prescription text), `.log-fields`/`.log-field`/`.log-field-lbl`/`.log-input` (real labeled stat-fields: small-caps label ABOVE, big Syne value, focus-glow ring, ghost last-session value as placeholder, green when filled — killed the old double-label button look). `checkExRowCoach` now toggles a `.done` class instead of setting inline border/bg (badge/PB logic unchanged; `[data-ss="1"].done` handles superset members). Applied to BOTH coach program rows (`updateLogWorkout`) and coach ad-hoc rows (`addAdHocExercise`). Verified rendered look via headless Chromium (idle/done/superset states) — `/opt/pw-browsers/chromium-1194/chrome-linux/chrome`, `NODE_PATH=/opt/node22/lib/node_modules`, reusable harness pattern in scratchpad (`preview.html`+`css_extract.css`+`shot.js`). **Client-portal logging (`renderWorkoutCard`, ~7920) is a SEPARATE render path still on the old inline-styled look — offered to mirror the same treatment there next (shared-feature rule) pending user approval of the direction.** Old `.workout-inputs`/`.wi-label` CSS left defined but now unused by the coach log.
- ✅ **Continuing Education — content audit (basics out, fresher topics in) + in-app "New" nudge** (`b055218`, `40907dd`). User: some tidbits felt too broad/basic ("stuff most trainers already know" makes people stop using it) and asked for more current/less-common material, plus wanted to explore notifications. (1) **Audit**: removed 6 foundational-obvious entries (progressive-overload, doms-not-goal, protein-total, dynamic-warmup, adherence, frequency); added 10 fresher ones — VO2max-as-mortality-predictor, isometric exercise for BP (2023 network meta-analysis), junk volume, myo-reps/rest-pause, intermittent-fasting-has-no-metabolic-magic, omega-3 + aging muscle, collagen+vit-C peri-loading for tendons, grip strength as a health biomarker, implementation intentions (if-then plans), muscle-as-endocrine-organ/myokines. Library now 64; same integrity rules (honest attributions, emerging work hedged, no fabricated stats). Done via `ce_audit.py` (Node extracts current `_CE_LIBRARY`→JSON, Python filters+appends+re-splices — reuse this for future audits since Python's json.loads chokes on the em-dash-heavy inline block, but Node `eval` of the array works). No-repeat rotation re-verified over two 64-day cycles. **Removed-id safety:** `_ceTodayEntry` filters removed ids from any persisted cycle and appends new ones; saved/reviewed state `.filter(Boolean)`s missing ids — no migration needed. (2) **In-app notification (zero backend):** `_ceTodayUnread()` + a "New" badge on the dashboard card that clears once today's topic is opened (reading marks it reviewed; `closeCE` re-renders the card). **TRUE web push (arrives when app is closed) is NOT built** — it needs a service worker + Web Push/VAPID + a Supabase subscriptions table + a daily Vercel Cron, and on iOS only works for the home-screen PWA on 16.4+ with granted permission. Presented to the user as a separate opt-in build; awaiting their decision.
...
**Last Updated:** HEAD `723de4b` — end of the session that shipped several functional dashboard/log fixes + a "Seated Hamstring Curl" catalog add + replaced Kathryn's program via SQL, then built and FULLY REVERTED a "data-lux" dashboard visual overhaul the user rejected on-device. index.html is at the pre-overhaul 5384720 content. Do not re-attempt the dashboard redesign unless the user explicitly asks. See the "Current Commit State" block up top + the top session-log entries.
