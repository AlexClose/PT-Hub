...
**Current HEAD** `3a8539d`: end of the big **audit + visual-identity session**. App state: all data-loss/analytics bugs from the 3-agent audit fixed; coach nav = bottom tab bar; native-feel pass done; ambient particle background v4 (near-transparent border-defined cards, 48–102 motes); Today gradient ring; glassy per-client avatars (golden-angle hues); mid-workout draft survival hardened. Everything ships straight to `main` → Vercel.
**File**: `index.html`, ~9000 lines (line numbers shift constantly — ALWAYS grep, never trust the numbers in this doc)
...
**Last Updated:** HEAD `3a8539d` — the **mega-session**: full 3-agent audit (23 findings fixed: lexicographic date-filter no-ops on TEXT `session_date` broke ALL monthly analytics; idempotent retry guards on every save path; saveEditSession insert-then-delete-by-id; calendar-strip tap ReferenceError; renew NaN; heatmap elapsed-weeks; +more — see log), "From Library" on-demand template fetch, coach **bottom tab bar** + slim brand nav + safe-area everywhere, native-feel pass, visual phases 3b–3e (client-modal declutter, emoji sweep, skeletons, stagger), phase 4 (view glow, surface lift, stat glow, **Today gradient ring**, per-client avatars → golden-angle hues → glassy tiles w/ hue glint), **ambient particle background v1→v4** (z-index architecture for iOS, ensure() restart, near-transparent border-only cards, 48–102 motes), and **mid-workout draft survival** (empty-save guard + `_tryFillFromDraft` on every log open + visibilitychange flush). **Open items for next session:** (1) user reacting to ambient v4 on phone (tune transparency/brightness/count — one number each); (2) client-portal Home hasn't had the atmosphere/visual treatment (coach-only so far — portal replaces `document.body`, needs its own canvas if wanted); (3) known-accepted audit items: concurrent coach+client `remaining` race (needs server-side RPC), shared-package overuse untracked; (4) muscle-diagram SVG swap still deferred; (5) ongoing `exerciseToMuscles`/demo calibration as user reports. **User's testing loop:** iPhone PWA, reads dates/analytics closely, reports visual taste verbally — implement, ship to main, they re-check.
